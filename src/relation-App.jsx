...
async function askClaude(messages, system = "") {
  ...
  // システムプロンプト＋会話履歴をGemini形式に変換
  const contents = messages.map((m) => ({
    role: m.role === "assistant" ? "model" : "user",
    parts: [{ text: m.content }],
  }));
  // systemがある場合は最初のユーザーメッセージの先頭に付加
...
// 1. AIチャット相談
// ─────────────────────────────────────────────────────────────
const CHAT_SYSTEM = `あなたは大学生の対人関係・学習の悩みに寄り添う親切なAIカウンセラー「RelationAI」です。
相談者の気持ちを受け止めながら、具体的で実践的なアドバイスを日本語で提供してください。
対人関係（教授・先輩・友人・グループワーク）、学習の進め方、過去問の入手方法なども相談できます。
回答は温かみがあり、簡潔で的確にしてください。マークダウンは使わず、自然な文章で。`;
...
// 2. 頼み方フレーズ生成
// ─────────────────────────────────────────────────────────────
const PHRASE_SYSTEM = `あなたは対人関係コミュニケーションの専門家です。
ユーザーの状況に応じた「頼み方フレーズ」を3パターン（丁寧・普通・カジュアル）生成してください。
以下のJSON形式のみで返してください（マークダウン・説明文不要）：
  ...
  const getAdvice = async (person) => {
    ...
    try {
      const text = await askClaude([{ role: "user", content: prompt }],
        "あなたは大学生の人間関係コーチです。具体的で実践的なアドバイスを日本語で。200字以内でまとめてください。");
...
試験・授業・資格勉強について、具体的で実践的なアドバイスを提供します。
時間管理・暗記法・過去問の活用法・勉強仲間の作り方なども含めて。
回答は親切で、自然な日本語で。箇条書きや改行を使って読みやすく。`;
