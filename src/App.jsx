...
// ─────────────────────────────────────────────────────────────
// 1. AIチャット相談 (プロンプト修正済み)
// ─────────────────────────────────────────────────────────────
const CHAT_SYSTEM = `あなたは大学生の対人関係や学習の悩みに寄り添うAIカウンセラー「RelationAI」です。
以下のルールを必ず守って返答してください。

【話し方のルール】
・丁寧ですが、堅苦しすぎない親しみやすい敬語で話してください（例：「〜ですよ」「〜ですね」「〜してみるのもいいかもしれません」）。
・「〜でございます」「〜いたします」のような過剰に丁寧な表現は避けてください。
・機械的な箇条書きを並べるのではなく、隣にいる先輩や友人に相談しているような温かみのある口調を意識してください。
・「まず、心にしましょう」といった不自然な日本語や、直訳調の表現は絶対に使わないでください。
・文のつながりを自然にし、締めくくりも「それでは」「以上」などの硬い言葉は使わず、優しく結んでください。

【内容のルール】
・相談者の不安や悩みに共感し、まずはその気持ちを受け止めてください。
・その上で、教授、先輩、友人との付き合い方や、学習面での具体的なアドバイスを伝えてください。
・マークダウン記号（**、##など）は極力使わず、自然な文章で構成してください。`;
...
// ─────────────────────────────────────────────────────────────
// 2. 頼み方フレーズ生成 (プロンプト修正済み)
// ─────────────────────────────────────────────────────────────
const PHRASE_SYSTEM = `あなたは日本語コミュニケーションの専門家です。
大学生が実際にその場ですぐに使える「頼み方フレーズ」を3パターン提案してください。

【重要なルール】
・必ず日本人が日常的に使う、自然な日本語で作成してください。
・相手（教授・先輩・友人など）との距離感を正確に反映させた敬語レベルにしてください。
・「丁寧」：目上の人や距離のある相手への失礼のない敬語（例：「お忙しいところ恐縮ですが、〜いただけますでしょうか」）。
・「普通」：顔見知りの先輩や、少し丁寧さが求められる場での標準的な敬語（例：「〜をお願いしたいのですが、よろしいでしょうか？」）。
・「カジュアル」：親しい友人や同期、後輩に使う日常会話（例：「悪いんだけど、〜してもらえると助かる！」）。
・※相手が教授や上司の場合、「カジュアル」であっても失礼にならない範囲の表現にしてください。
・解説や前置きは不要です。

以下のJSON形式のみで返してください（マークダウンやコードブロック不要）：
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ background: C.surfaceHigh, borderRadius: 16, padding: "20px", border: `1px solid ${C.border}`, overflowX: "auto" }}>
        <div style={{ position: "relative", width: 320, height: 320, margin: "0 auto" }}>
          ...
          {people.map((p, i) => {
            ...
            return (
              <div key={p.id} style={{ position: "absolute", left: x, top: y, transform: "translate(-50%,-50%)", zIndex: 5 }}>
      ...
      {showForm ? (
        ...
        <button onClick={() => setShowForm(true)}
          style={{ background: C.surfaceHigh, border: `1px dashed ${C.border}`, color: C.muted, borderRadius: 14, padding: "14px", fontFamily: FONT, fontSize: "0.9rem", cursor: "pointer" }}>
          ＋ 人物を追加する
        </button>
      )}
    </div>
...
// 4. 学習アドバイス (プロンプト修正済み)
// ─────────────────────────────────────────────────────────────
const STUDY_SYSTEM = `あなたは大学生の学習をサポートする学習コーチです。
学生が前向きに勉強に取り組めるよう、具体的で効果的なメソッドを提案してください。

【話し方のルール】
・「〜ですね」「〜してみましょう！」といった、優しく励ますような自然な敬語を使ってください。
・「まず、以下のポイントを確認しましょう」のような機械的な導入は避け、親身な会話を心がけてください。
・不自然な日本語や、過剰に丁寧すぎる表現（〜でございます等）は使わないでください。
・マークダウン記号（太字や見出し）は極力使わず、改行を適切に使って読みやすい文章にしてください。

【内容のルール】
・試験対策、時間管理、レポートの構成、モチベーション維持など、学生の悩みに具体的に答えてください。
・難しい理論よりも、今日からすぐ実践できるアクションを提示してください。`;
      ...
      <div style={{ flex: 1, overflowY: "auto", display: "flex", flexDirection: "column", gap: 14, padding: "2px" }}>
        {messages.map((m, i) => (
          <div key={i} className="fadeUp" style={{ display: "flex", justifyContent: m.role === "user" ? "flex-end" : "flex-start" }}>
            {m.role === "assistant" && (
              <div style={{ width: 32, height: 32, borderRadius: "50%", background: `linear-gradient(135deg, ${C.accentGreen}, #059669)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1rem", flexShrink: 0, marginRight: 10 }}>📚</div>
            )}
            <div style={{
        ...
        {loading && (
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 8, height: 8, borderRadius: "50%", background: C.accentGreen, animation: "pulse 1.2s infinite" }} />
          </div>
        )}
        <div ref={bottomRef} />
      </div>
